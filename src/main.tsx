...
function App() {
  ...
  const allocationTotal = selectedPortfolio?.assets.reduce((total, asset) => total + Math.max(asset.value ?? 0, 0), 0) ?? 0;
  const pricedAllocationAssets = (selectedPortfolio?.assets ?? []).filter(asset => (asset.value ?? 0) > 0).sort((left, right) => (right.value ?? 0) - (left.value ?? 0));
  const allocationLeaders = pricedAllocationAssets.slice(0, 5).map((asset, index) => {
    const symbol = displayAssetSymbol(asset);
    return { symbol, value: asset.value ?? 0, icon: CORE_ICONS[tokenKey(symbol)] || asset.icon, color: ALLOCATION_COLORS[tokenKey(symbol)] || ALLOCATION_FALLBACK_COLORS[index % ALLOCATION_FALLBACK_COLORS.length] };
  });
  const allocationRemainder = pricedAllocationAssets.slice(5).reduce((total, asset) => total + (asset.value ?? 0), 0);
